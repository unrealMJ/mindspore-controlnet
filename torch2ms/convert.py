...
def convert_torch_to_numpy(name='cond_stage_model', save=False):
...
def save_ms_ckpt(ckpt, name):
    ...
    ms.save_checkpoint(save_data, f'./ms_weight/{name}.ckpt')
...
def convert_unet():
    path = '/mnt/petrelfs/majie/project/ControlNet/convert/unet.pkl'
    with open(path, 'rb') as f:
        numpy_weight = pickle.load(f)
    ...
    save_data = []
    for k, v in ms_weight.items():
        save_data.append({
            'name': k,
            'data': v
        })

    ms.save_checkpoint(save_data, 'unet.ckpt')
...
if __name__ == '__main__':
    numpy_weight = convert_torch_to_numpy(name='control_model', save=True)
    ckpt = convert_controlnet(numpy_weight)
    save_ms_ckpt(ckpt, 'controlnet')
